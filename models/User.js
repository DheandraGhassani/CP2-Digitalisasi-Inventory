...
class User {
    ...
    static async create(userData) {
        try {
            const { name, email, password, role, roles_id, profile_photo = null } = userData;
            const hashedPassword = await bcrypt.hash(password, 10);
            
            const query = `
                INSERT INTO users (name, email, password, role, roles_id, profile_photo, is_active, created_at, updated_at)
                VALUES (?, ?, ?, ?, ?, ?, 1, NOW(), NOW())
            `;
            const result = await db.query(query, [name, email, hashedPassword, role, roles_id, profile_photo]);
    ...
    static async update(id, userData) {
        try {
            const { name, email, role, roles_id, profile_photo } = userData;
            const query = `
                UPDATE users 
                SET name = ?, email = ?, role = ?, roles_id = ?, profile_photo = ?, updated_at = NOW()
                WHERE id = ?
            `;
            await db.query(query, [name, email, role, roles_id, profile_photo, id]);
            return true;
        } catch (error) {
